...
/*   Updated: 2025/03/30 15:26:56 by albernar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_square_inner(t_game *game, t_minimap minimap, t_dvec2d xd)
{
	...
	while (tmp_y < MINIMAP_SQUARE_SIZE)
	{
		...
		process_pixel(game, index, minimap.tile);
		tmp_y++;
...
static void	draw_square(t_game *game, t_dvec2d m, char tile, t_fvec2d theta)
{
	int			tmp_x;
	t_dvec2d	xd;
	t_minimap	minimap;
		...
		while (incr.x < game->map.width - 1)
		{
				...
				<= mini_r * mini_r)
			{
				if (incr.x <= (int)ft_strlen(game->map.grid[incr.y]))
					draw_square(game, m, game->map.grid[incr.y][incr.x], theta);
			}
			incr.x++;
		}
		incr.y++;
