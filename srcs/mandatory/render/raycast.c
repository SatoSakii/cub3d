...
/*   Updated: 2025/03/30 15:37:12 by albernar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	t_ivec2d	tex;
	double		pos_and_step[2];
	t_textures	texture;

	texture = init_draw(draws, rdr.side, game);
	calculate_texture_coordinates(rdr.wall_x, &texture, rdr.side, &tex);
	pos_and_step[1] = (double)texture.height / (double)(rdr.line_height);
	pos_and_step[0] = ((draws[0] + 1) - (WINDOW_HEIGHT >> 1)
			+ (rdr.line_height >> 1)) * pos_and_step[1];
	while (++draws[0] <= draws[1])
	{
		tex.y = (int)pos_and_step[0];
		pos_and_step[0] += pos_and_step[1];
