...
/*   Updated: 2025/03/07 01:03:12 by albernar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

static bool	resize_map(t_game *game, t_error_ctx *ctx, size_t *size)
{
	char	**new_grid;
	...
	size_t *size, bool *valid_line)
{
	...
	*valid_line = true;
	...
	if (game->map.width < ft_strlen(ctx->line_content))
		game->map.width = ft_strlen(ctx->line_content);
	return (true);
}

static bool	init_grid(size_t size, t_error_ctx *ctx, t_game *game)
{
